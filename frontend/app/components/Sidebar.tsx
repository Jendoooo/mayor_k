...
import { WorkShift } from '@/app/lib/api';
...
export default function Sidebar({ activeShift, onEndShift }: SidebarProps) {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside className={`fixed left-0 top-0 bottom-0 w-64 bg-midnight-blue border-r border-white/10 flex flex-col z-40 text-slate-300 shadow-2xl transition-transform duration-300 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
                }`}>
                ...
                {/* Shift Section */}
                {activeShift && (
                    <div className="px-4 py-3 border-t border-white/10 bg-emerald-900/10">
                        ...
                        <button
                            onClick={onEndShift}
                            className="w-full py-2 bg-red-600/10 hover:bg-red-600 border border-red-600/30 text-red-400 hover:text-white rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2"
                        >
                            <LogOut size={14} />
                            End Shift
                        </button>
